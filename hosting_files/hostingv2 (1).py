    ...
    if not user_input:
        ...
        with st.status("Agent is working...", expanded=True) as status:
            ...
            def sanitize_metadata(metadata: dict) -> dict:
                ...
                This converts any non-compliant values to strings.
                """
                sanitized = {}
            ...
            def read_documents(urls):
                ...
                for url in urls:
                    try:
                        ...
                        # FIX: sanitize metadata so ChromaDB doesn't reject non-string values
                        chunk_docs = [
                            LCDocument(
                                page_content=t,
                                metadata=sanitize_metadata({"source": url['url']})
                            )
            ...
            # LLM for generating retrieval queries
            ...
            def generate_retrieval_queries(user_query: str, api_key: str) -> list:
            ...
            def multi_query_retrieve(retriever, queries: list, top_k: int = 10) -> list:
                """
            ...
            def verify_papers(papers):
                ...
                for paper in papers:
                    ...
                    if response.content.strip() in ['survey', 'comparison', 'method-specific', 'tutorial']:
            ...
            try:
                ...
                print(f"Error parsing task_n_prompt: {e}")
                x = "User query extraction failed."

            trimmed_info = str(req_info)[:20000] 
            ...
            def write_report():
                ...
                writer_messages = [
                    ...
                    HumanMessage(content=writer_human_prompt)
                ]
            ...
            # FIX: Save to buffer and reset pointer to start before reading
            pdf_buffer = BytesIO()
            pdf.save(pdf_buffer)
            pdf_buffer.seek(0)  # <-- critical fix: reset pointer before getvalue()
