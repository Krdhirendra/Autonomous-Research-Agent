...
if st.button("Run Research Agent"):
    if not user_input:
        ...
        with st.status("Agent is working...", expanded=True) as status:
            ...
            # 1. Fixed the model name to a valid Groq model
            writer_llm = ChatGroq(
                model='llama-3.3-70b-versatile', 
                api_key=GROQ_TOKEN,
                temperature=0.0,
            ...
            # 2. Removed the JSON requirement to allow for better document formatting
            ...
            # Extract variables (assuming your JSON parses correctly)
            try:
            ...
            # 3. Consider lowering the slice if you hit free-tier rate limits
            ...
            def write_report():
                ...
                # 4. Added error handling to catch API/Auth/Rate Limit issues
                try:
            ...
            # Save to a buffer instead of a local path for ho0sting
            pdf_buffer = BytesIO()
            pdf.save(pdf_buffer)
